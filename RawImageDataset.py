...
class MIMIC_raw(Dataset):
    def __init__(self, dataset_csv, resize=448, transform=True, upscale_to=None):
    ...
    def __getitem__(self, idx):
        img_root_dir = "/orcd/pool/006/lceli_shared/data/physionet.org/files/mimic-cxr-jpg/2.0.0/files/"
        ...
        if self.upscale_to is not None and self.upscale_to != self.resize:
            full_img = full_img.resize((self.upscale_to, self.upscale_to))
        ...
        output = {'img_dir': img_dir, 'full_img': full_img, 'insurance': output_insurance, 'img_id': self.rows[idx][0], 'gender': gender, 'age': age, 'race': race}
        return output
...
class MIMIC_raw_permuted_label(Dataset):
    """Like MIMIC_raw_random_label but with insurance labels shuffled across samples.

    Unlike MIMIC_raw_random_label (which draws from randn and gives ~50/50 split
    regardless of the true class balance), this class permutes the actual insurance
    labels so the marginal distribution is preserved while the label-image
    ...
    def __init__(self, dataset_csv, resize=448, diameter=50, transform=True):
        ...
        insurance_index = self.title.index("new_insurance_type")
        labels = [self.rows[i][insurance_index] for i in range(len(self.rows))]
        perm = list(range(len(labels)))
        random.shuffle(perm)
        self.permuted_labels = [labels[perm[i]] for i in range(len(labels))]
    ...
    def __getitem__(self, idx):
        ...
        insurance_type = self.permuted_labels[idx]
        gender_index = self.title.index("gender")
        gender = self.rows[idx][gender_index]
        age_index = self.title.index("anchor_age")
        age = float(self.rows[idx][age_index])
...
if __name__ == "__main__":
    dataset = MIMIC_raw("insurance_dataset_8_1_1_PMMthree_train_no_support_devices.csv", resize=4, transform=False)
