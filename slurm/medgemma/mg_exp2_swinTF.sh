...
EXPERIMENT_NAME="mg_exp2_swinTF"
...
echo "Exp2-2: SwinTF resolution (7 resolutions, MedGemma)"
echo "Note: SwinTF skips res=2 (patch size incompatibility)"
echo "========================================="

python run_exp2.py \
    --train_path $TRAIN --val_path $VAL --test_path $TEST \
    --experiment_name mg_swinTF_resize \
    --weight_dir mg_exp2/swinTF \
    --model swinTF \
    --root_dir $ROOT

### Collect weights
echo "Collecting weights..."
mkdir -p $HF_WEIGHTS/exp2-2_medgemma
for res in 4 7 14 28 56 112 224; do
    SRC=$ROOT/${res}_mg_exp2/swinTF/Rand123/Rand123_${res}_mg_swinTF_resize_model_aucbest.pt
    DST=$HF_WEIGHTS/exp2-2_medgemma/swinTF_${res}.pt
