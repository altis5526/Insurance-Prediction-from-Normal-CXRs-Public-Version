...
# Submit with: sbatch --export=VARIANT=t --job-name=mg_e1r_t slurm/medgemma/mg_exp1_mamba_remove_array.sh
#              sbatch --export=VARIANT=s --job-name=mg_e1r_s slurm/medgemma/mg_exp1_mamba_remove_array.sh
...
### Logging
LOG_DIR="slurm/medgemma/mg_exp1_mamba_${VARIANT}_remove"
mkdir -p "$LOG_DIR"
...
echo "Variant: mamba_${VARIANT}, Method: remove, Patch: ${PATCH}"
echo "========================================="

### Train
python train_insurance_fullimgsize_mamba_mask_area.py --mode train \
    --train_path $TRAIN --val_path $VAL \
    --experiment_name mg_mamba_${VARIANT}_remove_patch${PATCH} \
    --weight_dir mg_exp1/mamba_${VARIANT}/remove \
...
SRC=$ROOT/mg_exp1/mamba_${VARIANT}/remove/Rand123/Rand123_mg_mamba_${VARIANT}_remove_patch${PATCH}_model_aucbest.pt
DST=$HF_WEIGHTS/exp1-1_medgemma_${VARIANT}/remove_patch${PATCH}.pt
