...
# Exp1 keep-one-patch. Submit with: sbatch --job-name=mg_e1k_s_fix slurm/medgemma/mg_exp1_mamba_keep_s_fix_array.sh
...
### Logging
LOG_DIR="slurm/medgemma/mg_exp1_mamba_s_fix_keep"
mkdir -p "$LOG_DIR"
...
echo "Variant: mamba_s_fix (depths=[2,2,8,2]), Method: keep, Patch: ${PATCH}"
echo "========================================="

### Train
python train_insurance_fullimgsize_mamba_mask_mostarea.py --mode train \
    --train_path $TRAIN --val_path $VAL \
    --experiment_name mg_mamba_s_fix_keep_patch${PATCH} \
    --weight_dir mg_exp1/mamba_s_fix/keep \
...
SRC=$ROOT/mg_exp1/mamba_s_fix/keep/Rand123/Rand123_patchidx${PATCH}_mg_mamba_s_fix_keep_patch${PATCH}_model_aucbest.pt
DST=$HF_WEIGHTS/exp1-1_medgemma_s_fix/keep_patch${PATCH}.pt
