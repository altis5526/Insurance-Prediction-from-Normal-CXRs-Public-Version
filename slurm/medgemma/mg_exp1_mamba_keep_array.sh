...
# Submit with: sbatch --export=VARIANT=t --job-name=mg_e1k_t slurm/medgemma/mg_exp1_mamba_keep_array.sh
#              sbatch --export=VARIANT=s --job-name=mg_e1k_s slurm/medgemma/mg_exp1_mamba_keep_array.sh
...
### Logging
LOG_DIR="slurm/medgemma/mg_exp1_mamba_${VARIANT}_keep"
mkdir -p "$LOG_DIR"
...
echo "Variant: mamba_${VARIANT}, Method: keep, Patch: ${PATCH}"
echo "========================================="

### Train
python train_insurance_fullimgsize_mamba_mask_mostarea.py --mode train \
    --train_path $TRAIN --val_path $VAL \
    --experiment_name mg_mamba_${VARIANT}_keep_patch${PATCH} \
    --weight_dir mg_exp1/mamba_${VARIANT}/keep \
...
### Test
python train_insurance_fullimgsize_mamba_mask_mostarea.py --mode test \
...
### Collect weight
mkdir -p $HF_WEIGHTS/exp1-1_medgemma_${VARIANT}
SRC=$ROOT/mg_exp1/mamba_${VARIANT}/keep/Rand123/Rand123_mg_mamba_${VARIANT}_keep_patch${PATCH}_model_aucbest.pt
DST=$HF_WEIGHTS/exp1-1_medgemma_${VARIANT}/keep_patch${PATCH}.pt
