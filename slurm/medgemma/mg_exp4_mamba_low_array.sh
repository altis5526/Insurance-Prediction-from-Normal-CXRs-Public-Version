...
# Submit with: sbatch --export=VARIANT=t --job-name=mg_e4l_t slurm/medgemma/mg_exp4_mamba_low_array.sh
#              sbatch --export=VARIANT=s --job-name=mg_e4l_s slurm/medgemma/mg_exp4_mamba_low_array.sh
...
### Logging
LOG_DIR="slurm/medgemma/mg_exp4_mamba_${VARIANT}_low"
mkdir -p "$LOG_DIR"
...
echo "Variant: mamba_${VARIANT}, Direction: lowpass, Frequency: ${FREQ}"
echo "========================================="

### Train
python train_insurance_fullimgsize_mamba_low_pass.py --mode train \
    --train_path $TRAIN --val_path $VAL \
    --experiment_name ${FREQ}LowPass_mg_mamba_${VARIANT}_lowpass \
    --weight_dir ${FREQ}LowPass_mg_exp4/mamba_${VARIANT}/lowpass \
    --seed 123 --low_pass_diameter $FREQ \
    --root_dir $ROOT \
    --model_variant mamba_${VARIANT} \
    --pretrained_path $PT

### Test
python train_insurance_fullimgsize_mamba_low_pass.py --mode test \
...
### Collect weight
mkdir -p $HF_WEIGHTS/exp4_medgemma_${VARIANT}/lowpass
SRC=$ROOT/${FREQ}LowPass_mg_exp4/mamba_${VARIANT}/lowpass/Rand123/Rand123_${FREQ}LowPass_mg_mamba_${VARIANT}_lowpass_model_aucbest.pt
DST=$HF_WEIGHTS/exp4_medgemma_${VARIANT}/lowpass/${FREQ}Hz.pt
