...
# Exp4 lowpass filter sweep. Submit with: sbatch --job-name=mg_e4l_s_fix slurm/medgemma/mg_exp4_mamba_low_s_fix_array.sh
...
### Logging
LOG_DIR="slurm/medgemma/mg_exp4_mamba_s_fix_low"
mkdir -p "$LOG_DIR"
...
echo "Variant: mamba_s_fix (depths=[2,2,8,2]), Direction: lowpass, Frequency: ${FREQ}"
echo "========================================="

### Train
python train_insurance_fullimgsize_mamba_low_pass.py --mode train \
    --train_path $TRAIN --val_path $VAL \
    --experiment_name ${FREQ}LowPass_mg_mamba_s_fix_lowpass \
    --weight_dir ${FREQ}LowPass_mg_exp4/mamba_s_fix/lowpass \
...
### Collect weight
mkdir -p $HF_WEIGHTS/exp4_medgemma_s_fix/lowpass
SRC=$ROOT/${FREQ}LowPass_mg_exp4/mamba_s_fix/lowpass/Rand123/Rand123_${FREQ}LowPass_mg_mamba_s_fix_lowpass_model_aucbest.pt
DST=$HF_WEIGHTS/exp4_medgemma_s_fix/lowpass/${FREQ}Hz.pt
