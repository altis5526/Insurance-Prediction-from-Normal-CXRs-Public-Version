# python run_exp1-1.py --method remove --train_path insurance_dataset_8_1_1_PMMthree_train_no_support_devices.csv --val_path insurance_dataset_8_1_1_PMMthree_val_no_support_devices.csv --test_path insurance_dataset_8_1_1_PMMthree_test_no_support_devices.csv --experiment_name ModifyOptim_Mask_area_Delete_Support_Devices_8_1_1split_BS32_PMMthree_mamba_DoubleLinear_Lion1e-5_20251123 --weight_dir ModifyOptim_Mask_area_Delete_Support_Devices_8_1_1split_BS32_PMMthree_mamba_DoubleLinear_Lion1e-5_20251123
import subprocess
import os
import argparse
        ...
        for seed in seeds:
            for idx in range(1, 10):
                ...
                train_result = subprocess.run(
                    f'python train_insurance_fullimgsize_mamba_mask_area.py --mode train '
                ...
                if train_result.returncode == 0:
                    ...
                    print(f"{'='*70}\n")
                    subprocess.run(
                        f'python train_insurance_fullimgsize_mamba_mask_area.py --mode test '
    ...
    if args.method == "keep":
        for seed in seeds:
            for idx in range(1, 10):
                ...
                print(f"{'='*70}\n")
                train_result = subprocess.run(
                    f'python train_insurance_fullimgsize_mamba_mask_mostarea.py --mode train '
                    f'--train_path {args.train_path} --val_path {args.val_path} '
                    f'--experiment_name {args.experiment_name} --weight_dir {args.weight_dir} '
                    f'--seed {seed} --patch_idx {idx} --root_dir {args.root_dir}',
                    shell=True
                ...
                if train_result.returncode == 0:
                    ...
                    print(f"{'='*70}\n")
                    subprocess.run(
                        f'python train_insurance_fullimgsize_mamba_mask_mostarea.py --mode test '
                        f'--train_path {args.train_path} --val_path {args.test_path} '
                        f'--experiment_name {args.experiment_name} --weight_dir {args.weight_dir} '
                        f'--seed {seed} --patch_idx {idx} --root_dir {args.root_dir}',
                        shell=True
