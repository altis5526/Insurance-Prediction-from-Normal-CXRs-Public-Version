...
# frequency - Swin Transformer High/Low Pass
...
if __name__ == "__main__":
    ...
    seeds = [123]
    _default_frequencies = [1, 5, 10, 25, 50, 100, 200, 400]
    frequencies = [int(f) for f in args.frequencies.split(",")] if args.frequencies else _default_frequencies
    ...
    for seed in seeds:
        if args.pass_type == "high":
            for frequency in frequencies:
                ...
                train_result = subprocess.run(
                    f'python train_insurance_fullimgsize_swintransformer_high_pass.py --mode train '
                ...
                if train_result.returncode == 0:
                    ...
                    print(f"{'='*70}\n")
                    subprocess.run(
                        f'python train_insurance_fullimgsize_swintransformer_high_pass.py --mode test '
        ...
        if args.pass_type == "low":
            for frequency in frequencies:
                ...
                print(f"{'='*70}\n")
                train_result = subprocess.run(
                    f'python train_insurance_fullimgsize_swintransformer_low_pass.py --mode train '
                    f'--train_path {args.train_path} --val_path {args.val_path} '
                    f'--experiment_name {full_experiment_name} --weight_dir {full_weight_dir} '
                    f'--seed {seed} --frequency {frequency} --root_dir {args.root_dir}',
                    shell=True
                ...
                if train_result.returncode == 0:
                    ...
                    print(f"{'='*70}\n")
                    subprocess.run(
                        f'python train_insurance_fullimgsize_swintransformer_low_pass.py --mode test '
                        f'--train_path {args.train_path} --val_path {args.test_path} '
                        f'--experiment_name {full_experiment_name} --weight_dir {full_weight_dir} '
                        f'--seed {seed} --frequency {frequency} --root_dir {args.root_dir}',
                        shell=True
