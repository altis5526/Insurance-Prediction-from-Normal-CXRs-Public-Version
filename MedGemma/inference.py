...
parser.add_argument("--output_name", type=str, default="response_Gemma1.5")
...
if args.restart:
    with open(f"{check_type}_{args.output_name}.json", "r") as f:
        reponse_dict = json.load(f)
    index_to_be_delete = []
    key_to_be_delete = []
    for i in range(len(data_json)):
        if data_json[i]["dicom_id"] in reponse_dict.keys() and len(reponse_dict[data_json[i]["dicom_id"]].keys()) == 3:
            index_to_be_delete.append(i)

        elif data_json[i]["dicom_id"] in reponse_dict.keys() and len(reponse_dict[data_json[i]["dicom_id"]].keys()) == 2:
            if data_json[i]["dicom_id"] not in key_to_be_delete:
                key_to_be_delete.append(data_json[i]["dicom_id"])
...
for i, batch in enumerate(tqdm(dataloader, desc="Inference Progress")):
    ...
    if batch_count % 20 == 0:
        print(f"Saving intermediate results to {check_type}_{args.output_name}.json...")
        with open(f"{check_type}_{args.output_name}.json", "w") as f:
            json.dump(reponse_dict, f)

        with open(f"{check_type}_{args.output_name}.txt", "w") as f:
