...
if __name__ == "__main__":
    check_type = "test"
    response_SD_path = f"{check_type}_response_Gemma.json"
    response_NF_path = f"{check_type}_response_Gemma1.5.json"
    origin_csv_path = f"../insurance_dataset_8_1_1_PMMthree_{check_type}_no_support_devices.csv"

    no_device_ids = []
    no_finding_ids = []
    
    with open(response_SD_path, "r") as f:
        responses_SD = json.load(f)

    with open(response_NF_path, "r") as f:
        responses_NF = json.load(f) 

    for dicom_id, response in responses_SD.items():
        try:
            if response["PROMPT1"]["Answer"] == "No":
                no_device_ids.append(dicom_id)
        except:
            if response["PROMPT1"]["answer"] == "No":
                no_device_ids.append(dicom_id)

    for dicom_id, response in responses_NF.items():
        try:
            if response["PROMPT2"]["Answer"] == "No Finding":
                no_finding_ids.append(dicom_id)
        except:
            if response["PROMPT2"]["answer"] == "No Finding":
                no_finding_ids.append(dicom_id)

    if check_type == "train":
        manual_ids = ["5bf93b39-762874ba-653777b8-d7c7ea68-939261c1",
        ...
        manual_ids = ["07ebc3a9-cac1e76a-a8b68886-121f886b-ca6e5606",
        ...
        manual_ids = ["5f4a29c9-b8a2933a-035cbf27-44258304-71fc12dc"]

    no_device_ids.extend(manual_ids)
    print(f"no device data count: {len(no_device_ids)}")
    print(f"no finding data count: {len(no_finding_ids)}")

    with open(origin_csv_path, "r") as f:
        df = pd.read_csv(f)

    df = df[df["dicom_id"].isin(no_device_ids)]
    df = df[df["dicom_id"].isin(no_finding_ids)]

    print(f"final lnegth: {len(df)}")
    
    with open(f"../insurance_dataset_8_1_1_PMMthree_{check_type}_medgemmaChecked.csv", "w") as f:
