...
if __name__ == "__main__":
    ...
    parser.add_argument("--frequency", help='High pass frequency/diameter', type=int)
    ...
    encoder = SwinTDoubleLinear(use_checkpoint=True, num_classes=num_classes, img_size=448)
    encoder.to(device)
    ...
    img_size = 448
    if args.mode == "train":
        train_dataset = MIMIC_raw_high_pass(train_path, resize=img_size, diameter=args.frequency, transform=True)
        val_dataset = MIMIC_raw_high_pass(val_path, resize=img_size, diameter=args.frequency, transform=False)
    elif args.mode == "test":
        train_dataset = MIMIC_raw_high_pass(train_path, resize=img_size, diameter=args.frequency, transform=True)
        val_dataset = MIMIC_raw_high_pass(val_path, resize=img_size, diameter=args.frequency, transform=False)
