...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01)).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))).exp()
...
class SwinTDoubleLinear(nn.Module):
    def __init__(self, use_checkpoint=True, num_classes=1000, img_size=448):
        super(SwinTDoubleLinear, self).__init__()

        self.swintransformer = SwinTransformerV2(
            img_size=448,
            embed_dim=128,
            depths=[2, 2, 18, 2],
            num_heads=[4, 8, 16, 32],
            window_size=14,                          # valid for 448, close to pretrained 12
            pretrained_window_sizes=[12, 12, 12, 6], # tells the bias MLP what it was trained at
            use_checkpoint=True,
        )
...
class SwinTV2_addICD(nn.Module):
    def __init__(self, img_size=448, num_classes=2, dropout_prob=0, use_checkpoint=True):
        super(SwinTV2_addICD, self).__init__()

        self.swintransformer = SwinTransformerV2(img_size=img_size, num_classes=0, use_checkpoint=use_checkpoint)
        # Add a new classification head
        self.classifier = nn.Linear(768+6+512, num_classes, bias=True)
...
class SwinTV2_addonlyICD(nn.Module):
    def __init__(self, img_size=448, num_classes=2, dropout_prob=0, use_checkpoint=True):
        super(SwinTV2_addonlyICD, self).__init__()

        self.swintransformer = SwinTransformerV2(img_size=img_size, num_classes=0, use_checkpoint=use_checkpoint)
        # Add a new classification head
        self.classifier = nn.Linear(768+512, num_classes, bias=True)
        self.attention = CLAM_SB()
