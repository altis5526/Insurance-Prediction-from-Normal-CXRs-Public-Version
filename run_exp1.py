        ...
        for seed in seeds:
            for idx in range(1, 10):
                ...
                train_result = subprocess.run(
                    f'python train_insurance_fullimgsize_densenet_mask_area.py --mode train '
                ...
                if train_result.returncode == 0:
                    ...
                    print(f"{'='*70}\n")
                    subprocess.run(
                        f'python train_insurance_fullimgsize_densenet_mask_area.py --mode test '
    ...
    if args.method == "keep":
        for seed in seeds:
            for idx in range(1, 10):
                ...
                print(f"{'='*70}\n")
                train_result = subprocess.run(
                    f'python train_insurance_fullimgsize_densenet_mask_mostarea.py --mode train '
                    f'--train_path {args.train_path} --val_path {args.val_path} '
                    f'--experiment_name {args.experiment_name} --weight_dir {args.weight_dir} '
                    f'--seed {seed} --patch_idx {idx} --root_dir {args.root_dir}',
                    shell=True
                ...
                if train_result.returncode == 0:
                    ...
                    print(f"{'='*70}\n")
                    subprocess.run(
                        f'python train_insurance_fullimgsize_densenet_mask_mostarea.py --mode test '
                        f'--train_path {args.train_path} --val_path {args.test_path} '
                        f'--experiment_name {args.experiment_name} --weight_dir {args.weight_dir} '
                        f'--seed {seed} --patch_idx {idx} --root_dir {args.root_dir}',
                        shell=True
