...
from swintransformer import SwinTransformerV2, SwinTDoubleLinear_addDemothen2
...
if __name__ == "__main__":
    ...
    encoder = SwinTDoubleLinear_addDemothen2(use_checkpoint=True, num_classes=num_classes, demo_size=demo_length)
    encoder.to(device)

    checkpoint = torch.load(pretrained_path, map_location=device)
    state_dict = checkpoint["model"] if "model" in checkpoint else checkpoint
    # Remove position buffers — they are recomputed at __init__ for the new window_size
    keys_to_remove = [k for k in state_dict.keys() 
                    if any(x in k for x in ['relative_coords_table', 
                                            'relative_position_index', 
                                            'attn_mask',
                                            'head.'])]  # head has wrong num_classes too
    for k in keys_to_remove:
        del state_dict[k]
    # Only load the backbone, not the head (head dims won't match)
    encoder.swintransformer.load_state_dict(state_dict, strict=False)
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, worker_init_fn=seed_worker, num_workers=4, shuffle=True, generator=g, persistent_workers=True, prefetch_factor=2)
    ...
    if training == True:
        ...
        with open(status_file, 'w') as f:
            ...
            f.write(f"MODEL: SwinTransformer\n")
