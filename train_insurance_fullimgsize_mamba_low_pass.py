...
if __name__ == "__main__":
    ...
    parser.add_argument("--low_pass_diameter", help='', type=int)
    ...
    train_dataset = MIMIC_raw_low_pass(train_path, resize=args.resize, diameter=args.low_pass_diameter, transform=True)
    val_dataset = MIMIC_raw_low_pass(val_path, resize=args.resize, diameter=args.low_pass_diameter, transform=False)
    ...
    if training == True:
        ...
        if start_epoch == 0:  # Only write fresh status if not resuming
            with open(status_file, 'w') as f:
                ...
                f.write(f"MODEL: Mamba (Low Pass Filter)\n")
                f.write(f"EXPERIMENT: {train_wandb_name}\n")
                f.write(f"START_TIME: {training_start_time}\n")
                f.write(f"LOW_PASS_DIAMETER: {args.low_pass_diameter}\n")
        ...
        config.low_pass_diameter = args.low_pass_diameter
