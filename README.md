...
AUC mean [95% CI] on the test split. MedMamba (scratch), DenseNet, SwinTF use bootstrap CIs; MedMamba-T / MedMamba-S point estimates are reported while runs complete (TBD = pending). Bold = best per row.
...
> Fill in MedMamba-T and MedMamba-S columns as results complete.
