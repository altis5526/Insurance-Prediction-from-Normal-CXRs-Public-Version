        ...
        for demo_input in combinations(demo, 1):
            ...
            if is_experiment_completed(args.root_dir, w_dir, exp_name, seed):
                ...
                train_result = subprocess.run(
                    f"python train_insurance_fullimgsize_densenet_addDemo_new.py --mode train "
                    f"--train_path {args.train_path} --val_path {args.val_path} "
                    f"--experiment_name {exp_name} --weight_dir {w_dir} "
                    f"--seed {seed} --demo_labels {demo_input[0]} "
                ...
                if train_result.returncode == 0:
                    subprocess.run(
                        f"python train_insurance_fullimgsize_densenet_addDemo_new.py --mode test "
        ...
        for demo_input in combinations(demo, 2):
            ...
            if is_experiment_completed(args.root_dir, w_dir, exp_name, seed):
                ...
                train_result = subprocess.run(
                    ...
                    f"--train_path {args.train_path} --val_path {args.val_path} "
                    f"--experiment_name {exp_name} --weight_dir {w_dir} "
                    f"--seed {seed} --demo_labels {demo_input[0]} {demo_input[1]} "
                    f"--root_dir {args.root_dir}",
                    shell=True
                )
        ...
        for demo_input in combinations(demo, 3):
            ...
            if is_experiment_completed(args.root_dir, w_dir, exp_name, seed):
                ...
                train_result = subprocess.run(
                    ...
                    f"--train_path {args.train_path} --val_path {args.val_path} "
                    f"--experiment_name {exp_name} --weight_dir {w_dir} "
                    f"--seed {seed} --demo_labels {demo_input[0]} {demo_input[1]} {demo_input[2]} "
                    f"--root_dir {args.root_dir}",
                    shell=True
                )
