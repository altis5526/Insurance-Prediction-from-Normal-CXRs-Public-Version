...
class MIMIC_raw(Dataset):
    def __init__(self, dataset_csv, resize=448, transform=True, img_root=None):
        ...
        self.img_root = img_root or "/datasets/mimic/mimic-cxr-jpg/2.0.0/files/"
...
class MIMIC_raw_ICD(Dataset):
    ...
    def __getitem__(self, idx):
        img_root_dir = "/datasets/mimic/mimic-cxr-jpg/2.0.0/files/"
        img_dir = f"p{self.rows[idx][1][:2]}/p{self.rows[idx][1]}/s{self.rows[idx][2]}/{self.rows[idx][0]}.jpg"
