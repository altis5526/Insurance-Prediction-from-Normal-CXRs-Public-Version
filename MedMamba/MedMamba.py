...
class PatchMerging2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # Pad to even spatial dimensions so stride-2 sampling is lossless.
        # This is the standard Swin Transformer fix for odd-sized feature maps
        # (e.g. 7×7 produced from 56×56 or 112×112 inputs).
        if (H % 2 == 1) or (W % 2 == 1):
            # F.pad order: (last_dim_left, last_dim_right, ..., first_dim_left, first_dim_right)
            # x is (B, H, W, C) so we pad dim-2 (W) then dim-1 (H); channels (dim-3) are untouched.
            x = F.pad(x, (0, 0, 0, W % 2, 0, H % 2))

        # Re-read potentially-updated spatial dims after padding
        _, H, W, _ = x.shape
...
class VSSM_addage(nn.Module):
    ...
    def forward(self, x, age, race):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x,start_dim=1)
        x = torch.cat((x, age, race), dim=-1)
        x = self.head(x)
        
        return x


class VSSM_addage5then2(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
        ...
        self.classifier2 = nn.Linear(5, num_classes) if num_classes > 0 else nn.Identity()
...
class VSSM_Double_addDemothen2(nn.Module):
    """Mamba addDemo variant with deeper classification head (matches JAMA training checkpoints)."""
    def __init__(self, demo_size=3, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
...
class VSSM_addonlyrace(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
    ...
    def forward(self, x, race):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x,start_dim=1)
        x = torch.cat((x, race), dim=-1)
        x = self.head(x)
        
        return x

class VSSM_addonlysex(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
...
class VSSM_addICD(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
        ...
        # self.norm = norm_layer(self.num_features)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.head = nn.Linear(self.num_features + 6 + 512, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, age, race, icd_embed):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x,start_dim=1)
        x = torch.cat((x, age, race, icd_embed), dim=-1)
        x = self.head(x)
        
        return x


class VSSM_addonlyICD(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
        ...
        # self.norm = norm_layer(self.num_features)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.head = nn.Linear(self.num_features + 512, num_classes) if num_classes > 0 else nn.Identity()
