...
from MedMamba.MedMamba import VSSM_Double_addDemothen2 as medmamba
import argparse
...
def evaluate(model, val_loader, num_classes):
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            # _, age_label = torch.max(test_age, 1)
            # _, race_label = torch.max(test_race, 1)
            # filter = (race_label == 2)
            # filter = (age_label == 2)
            # filter = (gender == 1)
...
if __name__ == "__main__":
    ...
    train_dataset = MIMIC_raw(train_path, resize=args.resize, transform=True)
    val_dataset = MIMIC_raw(val_path, resize=args.resize, transform=False)
